...
def load_adj_PEMS():
    A = pd.read_csv ("//root/Lane_level/AGWN/Data_process/data/PEMS_adj_43.csv", index_col=0)
    A = np.array(A)
...
def load_adj_HuanNan():
    A = pd.read_csv ("/root/Lane_level/AGWN/Data_process/data/Huanan_adj.csv", index_col=0)
    A = np.array(A)
    adj_mx = A.astype(np.float32)
    # adj = [asym_adj(adj_mx), asym_adj(np.transpose(adj_mx))]
